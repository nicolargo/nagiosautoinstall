...
def nagiosupdate():
  ...
  showexec ("Hack for Nagios 4.0 and 4.0.1" ,
            "apt-get install daemon && if [ ! -e /etc/rc.d/init.d/functions ]; then sed -i 's/^\.\ \/etc\/rc.d\/init.d\/functions$/\.\ \/lib\/lsb\/init-functions/g' /etc/init.d/nagios; sed -i 's/status\ /status_of_proc\ /g' /etc/init.d/nagios; fi", 1)
